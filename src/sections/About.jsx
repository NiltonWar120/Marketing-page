...
export default function About() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Text reveal
      gsap.fromTo(textRef.current.querySelectorAll('.about-reveal'),
        { opacity: 0, y: 50 },
      ...
      // Floating numbers
      gsap.to('.about-float', {
        y: -20, duration: 4, ease: 'sine.inOut', yoyo: true, repeat: -1, stagger: 0.5
      })
    }, sectionRef)
    return () => ctx.revert()
  }, [])
        ...
        <div className="about-visual">
          <div className="about-img-wrap" ref={imgRef}>
            ...
            <div className="about-float float-1">
              <span className="display-md" style={{ color: 'var(--accent)', opacity: 0.6 }}>N</span>
            </div>
            <div className="about-float float-2">
              <span className="label" style={{ color: 'var(--white-dim)' }}>Est. 2019</span>
            </div>
          </div>

          <div className="about-year-badge glass">
            <span style={{ fontFamily: 'var(--font-display)', fontSize: '3rem', lineHeight: 1, color: 'var(--accent)' }}>5+</span>
            <span className="label">Años de<br />excelencia</span>
          </div>
        </div>

        <div className="about-text" ref={textRef}>
          <div className="about-reveal pill" style={{ marginBottom: '2rem', opacity: 0 }}>
            <span className="dot" />
            <span>Sobre NEXUS</span>
          </div>

          <h2 className="about-reveal display-md" style={{ opacity: 0, marginBottom: '2rem' }}>
            No hacemos sitios web.<br />
            <em style={{ fontStyle: 'normal', color: 'var(--gold)' }}>Construimos activos digitales.</em>
          </h2>

          <p className="about-reveal body-lg" style={{ opacity: 0, marginBottom: '1.5rem' }}>
          ...
          <p className="about-reveal body-lg" style={{ opacity: 0, marginBottom: '3rem' }}>
          ...
          <div className="about-reveal about-values" style={{ opacity: 0 }}>
            {['Obsesión por el detalle', 'Resultados antes que estética', 'Transparencia total', 'Entrega sin excusas'].map((v, i) => (
